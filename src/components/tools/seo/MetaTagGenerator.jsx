    ...
    return (
        <ToolLayout title="Meta Tag Generator" description="Generate SEO-friendly meta tags for your website. Create title, description, keywords, and Open Graph tags." keywords={['meta tag generator', 'SEO tags', 'meta description generator', 'title tag generator']} category="seo" categoryName="SEO & Webmaster" faqs={faqs} relatedTools={relatedTools} seoContent={seoContent}>
            <div className="tool-form">
                <div className="form-group"><label className="form-label">Page Title ({title.length}/60)</label><input type="text" className="form-input" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Your page title" maxLength="70" /></div>
                <div className="form-group"><label className="form-label">Description ({description.length}/160)</label><textarea className="form-input" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Your page description" maxLength="200" rows={3} /></div>
            ...
            <style>{`.tool-form{max-width:700px;margin:0 auto}.form-row{display:grid;grid-template-columns:1fr 1fr;gap:var(--spacing-md)}.form-input{width:100%;padding:var(--spacing-sm);border:2px solid var(--platinum);border-radius:var(--radius);font-family:inherit}.checkboxes{display:flex;gap:var(--spacing-lg);margin-bottom:var(--spacing-lg)}.checkboxes label{display:flex;align-items:center;gap:var(--spacing-sm);cursor:pointer}.code-output{background:var(--bg-dark);color:#f0f0f0;padding:var(--spacing-lg);border-radius:var(--radius);position:relative;margin-top:var(--spacing-lg)}.code-output pre{font-family:var(--font-mono);font-size:var(--text-sm);white-space:pre-wrap;word-break:break-all;margin:0}.copy-btn{position:absolute;top:var(--spacing-sm);right:var(--spacing-sm);background:var(--yinmn-blue);color:white;border:none;padding:var(--spacing-xs) var(--spacing-sm);border-radius:var(--radius);cursor:pointer}`}</style>
        </ToolLayout>
    );
};
